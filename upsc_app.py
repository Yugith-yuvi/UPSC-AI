...
# --- PAGE 2: PRELIMS PYQ QUIZ ---
    ...
    st.markdown('<div class="hero-sub">Practice questions from your question bank, filtered by subject and year range.</div>', unsafe_allow_html=True)
    ...
    if st.button("Generate Quiz Test 🚀", type="primary", use_container_width=True, key="btn_gen_prelims"):
        ...
        with st.spinner("Fetching questions..."):
            try:
                ...
                payload = res.json()
                st.session_state.prelims_questions = payload.get("data", [])
            except requests.RequestException as exc:
                st.error(
                    "Could not contact the quiz backend. Start the backend and try again. "
                    f"Details: {exc}"
                )
            except (ValueError, TypeError) as exc:
                st.error(f"The backend returned an unexpected response: {exc}")

        if not st.session_state.prelims_questions:
            st.warning(
                f"No questions found for {subject} from {years[0]} to {years[1]}. "
                "Add verified questions for this subject and year range to your backend database."
            )
